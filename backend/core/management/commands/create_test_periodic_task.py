...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            self.stdout.write(self.style.WARNING(f"Interval schedule already exists"))
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f"Successfully created periodic task: {task.name}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"Periodic task already exists: {task.name}")
            )
