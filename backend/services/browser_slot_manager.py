...
class BrowserSlotManager:
    ...
    def get_instance(cls) -> "BrowserSlotManager":
        ...
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    max_concurrent = int(
                        os.environ.get("BROWSERBASE_MAX_CONCURRENT", "1")
                    )
    ...
    def acquire_slot(self, device: str, browser: str) -> None:
        ...
        with self.condition:
            ...
            print(
                f"[SLOT MANAGER] {device}/{browser}: Acquired slot ({self.active_sessions}/{self.max_concurrent} active)"
            )
        ...
        for attempt in range(max_retries):
            try:
                session = bb.sessions.create(
                    project_id=project_id, browser_settings=browser_settings
                )
                print(
                    f"[SLOT MANAGER] {device}/{browser}: Session created successfully (attempt {attempt + 1})"
                )
                return session
                ...
                if (
                    ...
                    if attempt < max_retries - 1:
                        print(
                            f"[SLOT MANAGER] {device}/{browser}: Rate limited (429), waiting {retry_after}s before retry {attempt + 1}/{max_retries}"
                        )
                        time.sleep(retry_after)
                        continue
                    else:
                        raise Exception(
                            f"Rate limit exceeded after {max_retries} attempts: {error_str}"
                        )
                else:
